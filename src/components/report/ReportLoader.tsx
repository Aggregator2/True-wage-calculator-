...
export default function ReportLoader({ size = 180, text }: LoaderProps) {
  const [stageIndex, setStageIndex] = useState(0);
  const displayText = text || STAGES[stageIndex];
  const letters = displayText.split('');

  useEffect(() => {
    if (text) return; // Don't cycle if text is manually set
        ...
        <div className="flex items-center justify-center gap-0.5">
          {letters.map((letter, index) => (
            <span
              ...
              className="inline-block text-white text-xl font-medium"
              style={{
                opacity: 0.4,
